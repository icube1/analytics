...
## 6. Тестовый слот `test.gala-soft.ru`

Каждый pull request в `master` (и ручной `Deploy test`) собирает Next standalone
и выкладывает его в `/opt/analytics-test` на порту `3001`. Один слот: последний
успешный деплой перезаписывает предыдущий.

Нужна DNS-запись:

| Имя | Тип | Значение |
|-----|-----|----------|
| `test.gala-soft.ru` | A | `5.253.30.126` |

Пока записи нет, Let's Encrypt сертификат не выпустится. После появления A-записи
следующий test-deploy попробует получить сертификат и включить HTTPS.

Вход тот же админский аккаунт (`/login`, логин `admin`). В шапке жёлтая полоса
«Тестовая среда» с именем ветки.
